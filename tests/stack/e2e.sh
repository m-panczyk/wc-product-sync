...
# --- Phase 11: price modifier — % + fixed amount + rounding (issue #14) ---------------------
#
...
echo "==> Phase 11: price modifier (% + fixed + rounding) (issue #14)"
swp eval '
foreach ( get_posts(array("post_type"=>array("product","product_variation"),"post_status"=>"any","numberposts"=>-1,"fields"=>"ids")) as $id ){ wp_delete_post($id,true); }
...
echo
echo "e2e PASS (sync + force-full + image + empty-source + undo + adopt + channel + bg-dry + bg-adopt + total-sync + total-refuse + price-mod)"
